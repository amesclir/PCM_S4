...
Necesitamos hacer algunos ajustes para conseguir tener dos filogenias y sus respectivos dataset de fenotipos en los que coincidan las especies que tenemos en las filogenias y en los datasets.
...
En este caso no vamos a usar directamente uno y otro rasgos, en cambio haraemos un PCA filogenetico de todos los rasgos disponibles.
...
Vamos a utilizar en nuestros análisis el PC3 que está directamente relacionado con la longitud de la extremidad delantera y trasera e inversametne relacionado con el número de laminillas que expansiones en forma de escama de los dedos y son claves en la típica escalada de estos animales. Las especies de este grupo que viven en árboles (escaladoras) suelen tener extremidades más cortas y mayor número de laminillas por lo que el PC# es de sumo interés para estudiar el microhabitat de estos organismos.

Para trabajar con OUwie lo primero que tenemos que hacer es fabricar un data.frame con los nombres de nuestras especies, el óptimo al que pertnece cada especie en nuestro análisis (en nuestro caso los diferentes microhabitat) y nuestro caracter continuo (PC3).
...
Finalmente vamos a ajustar nuestro modelos. Ajustaremos un modelo browniano simple, un modelo browniano en el que sigma2 cambia en función del microhabitat y finalmente un modelo OU en el que el optimo theta cambia en función del microhábiat.
...
Veamos un gráfico donde podemos apreciar claramente el proceso de convergencia evolutiva que se en este grupo.
...
MODELOS BROWNIANOS CON MÚTIPLES VARIABLES Y CORRELACIONES EVOLUTIVAS
...
Esto nos muestra que el mejor modelo, la correlacion coevolutiva between nuestras dos variables cuantitativas es mucho mayor en peces piscivoros quen en no piscivoros. Esto tiene sentido biológico ya que en los peces piscivoros es necesario una integración mucho mayor de los diferentes aspectos del aparato de alimentación.
...
Una primera posibiliad es que haya cambios bruscos en la tasa de evolución en un punto o varios puntos de la filogenia. Pero en este caso en vez de eso cambios estar marcados a priori por un caracter discreto, la localización del dichos cambios será también estimada.
Esto lo podemos hacer con la función rateshift de phytools.
Para comprobar esto vamos a utilizar un dataset de Broeckhoven et al., (2016) en la que los autores estudian la evolución de las estructura de defensa en una familia de lagartos sudafricanos, Cordylidae. Usaremos la función mencionada para ajustar un modelo en la que la tasa de cambio descenso de manera brusca a partir de un momento (En el estudio original se ajusto un modelo EB o early burst en la que tasa de evolución es máxima en la raiz del árbol y luego decae progresivamente hasta el presente).
...
Ahora vamos a ajustar modelos brownianos simples, with un cambio en la tasa y con dos cambios en la tasa de evolución.
...
El mejor modelo es el que tiene 2 tasas evolutivas (aunque muy cercano al que tiene una sola tasa evolutiva).
...
En este caso usamos rjMCMC (reversible jump Markov chain Monte Carlo). Es similar a MCMC pero en este caso no solo muestreamos diferentes valores de los parametros en la cadenas de Markov para calcular su distribución a posteriorin en este caso incluso saltamos entre diferentes altenativas de conjunto de parámetros en los modelos muestreados.
...
Este tipo de análisis han aparecido recientemente (Eastman et al., 2011; Pagel and Meade 2013) pero cada vez se están haciendo más y más populares.
Hay diferentes implementaciones de modelos mediante rjMCMC. Nosotros revemos una de ellas, en concreto la que está implementada en el paquete bayou de R. En este paquete se pueden inferir mediante rjMCMC el número de cambos de optimo en modelos OU más o menos complejos.
...
La configuración de dalpha="dhalfcauchy" asigna una distribución a priori tipo half-Cauchy parámetro alpaha del modelo OU. Del mismo modo, el argumento param es una lista de listas que se pueden usar para establecer las distribuciones a priori de los diferente parámetros. Entonces, por ejemplo, configurar dalpha=list(scale=0.1) le dice a make.prior que parametrice nuestra distribución a priori half-Cauchy para alpha usando un valor de parámetro de escala 0.1.
...
Podemos incluso calcular la probabilidad a priori del modelo con de los valores iniciales elegidos al azar.
...
Ahora vamos a descartar las primeras iteraciones del análisis que en cualquier análisis de tipo MCMC son subóptimas. Deberíamos de tener al menos 2 análisis para evaluar convergencia y estacionariedad y decidir el valor de burn-in pero por ahora vamos a descartar simplemente el 30% inicial del análisis.
...
Repita el análisis de Anolis en este capítulo con el paquete OUwie, pero esta vez analice PC1 en lugar de PC3. Describe y cuantifica cómo difieren tus resultados.
