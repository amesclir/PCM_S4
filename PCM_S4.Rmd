...
En este caso no vamos a usar directamente uno y otro rasgos, en cambio haraemos un PCA filogenetico de todos los rasgos disponibles (SVL, HL, HLL, FLL, LAM y TL).
...
Para trabajar con OUwie lo primero que tenemos que hacer es fabricar un data.frame con los nombres de nuestras especies, el óptimo al que pertnece cada especie en nuestro análisis (en nuestro caso los diferentes microhabitats) y nuestro caracter continuo (PC3).
...
Finalmente vamos a ajustar nuestro modelos. Ajustaremos un modelo browniano simple, un modelo browniano en el que sigma2 cambia en función del microhabitat y finalmente un modelo OU en el que el óptimo theta cambia en función del microhábiat.
